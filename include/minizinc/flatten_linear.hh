...
/// Linear expressions: building them, normalising them, and deciding which of
/// them keep a variable of their own.
///
/// A linear expression is a `lin_exp(coefficients, variables, constant)` call.
/// Flattening keeps an expression of more than one term behind a variable, and
/// leaves a comparison between two of those under a name of the compiler's own
/// (see linear_barrier and deferred_relation_id) rather than building it. Once
/// the model is complete, the redefinition loop in flatten() builds those
/// comparisons and decides, for each expression, whether the variable pays for
/// itself.
...
/// Return the deferred relation for \a bot, swapping greater-than operands.
ASTString deferred_relation_id(BinOpType bot, bool& swapOperands);

/// Whether \a id is one of the relations deferred_relation_id hands out.
...
/// Return the comparison the linear relation \a c stands for, or nullptr.
/// \a reifiedInto is set to the variable \a c is reified into, if any.
...
/// that the names in it are decided. \a reifiedInto is what to bind the result
/// to, or nullptr to take it from \a i.
void rebuild_linear(EnvI& env, Item* i, Call* c, Expression* e, VarDecl* reifiedInto);

/// Build the definition held by \a vdi into every supported reader.
