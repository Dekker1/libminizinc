...
Bug fixes:
^^^^^^^^^^
-  Fix a bug where expressions generated by ``regular(var int, string)`` might
   be incorrectly unified when paths are enabled.
...
Changes in the IDE:
^^^^^^^^^^^^^^^^^^^
-  Fix highlighting of the ``list`` keyword (:idebugref:`227`).
