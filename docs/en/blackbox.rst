...
The decision-variable arguments of a propagator are restricted to **at most one**
:mzn:`array[int] of var int` and **at most one** :mzn:`array[int] of var float`
(``list of var int`` / ``list of var float``). Any number of fixed (parameter)
arguments may also be given; these are not passed to the external function but
may be used by the companion functions described below.

Value propagators
~~~~~~~~~~~~~~~~~~

A **relational** value propagator is a predicate. It is scheduled once all of
its variables are fixed and reports whether the assignment satisfies the
constraint:

.. code-block:: minizinc

  predicate all_ok(array[int] of var int: xs)
      ::minizinc_value_propagator
      ::blackbox_exec("./checker");

  constraint all_ok(x);

A **functional** value propagator is a function returning
:mzn:`array[int] of var int` or :mzn:`array[int] of var float`; the external
function computes the output values rather than merely checking them. Because
the length of the result cannot be known in general, a functional propagator
must also provide a ``par`` overload of the same function — the actual
propagation function — which the compiler evaluates on the *lower bounds* of the
inputs to determine the index set of the output:

.. code-block:: minizinc

  % par overload: the propagation function, used to size the output
  function array[int] of int: transform(array[int] of int: xs) =
      [ xs[i] + i | i in index_set(xs) ];

  % the black-box declaration (no body)
  function array[int] of var int: transform(array[int] of var int: xs)
      ::minizinc_value_propagator
...
.. code-block:: minizinc
  ...
  predicate my_bounds(array[int] of var int: xs)
...
.. code-block:: minizinc
  ...
  function array[int] of tuple(int,
                               array[int] of tuple(int, PropBnd),
                               array[int] of tuple(int, PropBnd)):
      my_bounds_reason(array[int] of var int: xs) =
...
exactly which input bounds the propagator inspects — the bounds that appear
anywhere in it. A solver uses this to schedule the propagator precisely, waking
it only when one of those bounds changes rather than on every change to an input
bound. With the conservative default reason every input bound is listed, so the
propagator is woken whenever any input bound changes.
...
.. code-block:: minizinc
  ...
  annotation blackbox_exec(string: executable, array[int] of string: args);
...
.. code-block:: minizinc
  ...
  predicate q(array[int] of var int: xs)
