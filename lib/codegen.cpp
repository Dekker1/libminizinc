...
void call_binop(CodeGen& cg, CG_Builder& frag, Mode ctx, BinOpType op, int r_lhs, int r_rhs) {
  switch(op) {
    ...
    case BOT_PLUS:
      ...
      return;
    case BOT_MINUS:
      PUSH_INSTR(frag, BytecodeStream::CALL, ctx, cg.find_builtin("int_minus"), CG::r(r_lhs), CG::r(r_rhs));
      return;
    case BOT_MULT:
...
private:
  ...
  void vFunctionI(FunctionI* f) {
    GCLock l;
    ASTString exp_str("export");
    if(f->ann().containsCall(exp_str)) {
...
CG::Binding bind_sum(Call* call, Mode ctx, CodeGen& cg, CG_Builder& frag) {
  ...
  PUSH_INSTR(frag, BytecodeStream::CALL, ctx, cg.find_builtin("int_plus"), CG::r(r_elt), CG::r(r_acc));
  // CLOSE_AGG(cg, frag);
