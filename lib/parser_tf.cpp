...
/* Lower DataZinc reduction events from tree-feller directly into the AST. */
...
public:
  ...
  /// Report where tree-feller stopped when tree-sitter cannot improve the diagnostic.
  void reportParseError(const TFError& err);
...
private:
  ...
  const char* _buf;
  unsigned int _len;
  /// Hold items until parsing succeeds; Model also keeps them rooted for GC.
  Model* _items;
  ASTString _filename;
  bool _ascii;
...
ParserLocation Lowerer::loc(unsigned int startByte, TFPoint start, unsigned int endByte,
  ...
  return {_filename, start.row + 1, firstColumn, end.row + 1, lastColumn};
}
...
void Lowerer::reportParseError(const TFError& err) {
  unsigned int col = codePointColumn(err.byte - err.point.column, err.byte) + 1;
  ParserLocation l(_filename, err.point.row + 1, col, err.point.row + 1, col);
  error(l, err.byte, std::string("syntax error, ") + err.message);
}
...
Expression* Lowerer::callExpr(const TFReduction* r, const ParserLocation& l) {
  ...
  // parser_ts.cpp:callExpr, minus the forms the DataZinc grammar cannot produce.
...
void parse_tree_feller(ParserState& pp) {
  ...
  // Commit only after success because the fallback reparses the whole file.
  std::unique_ptr<Model> items(new Model);
  bool hadError = pp.hadError;
  size_t errorCount = pp.syntaxErrors.size();
  size_t callCount = pp.dataFileCalls.size();

  Lowerer lowerer(pp, syms(), items.get());
  TFSink sink = {&lowerer, nullptr,
                 ...
                 nullptr};
  TFError error;
  if (!tf_parse(datazinc_tables(), pp.buf, pp.length, &sink, nullptr, &error)) {
    pp.hadError = hadError;
    pp.syntaxErrors.erase(pp.syntaxErrors.begin() + static_cast<long>(errorCount),
                          pp.syntaxErrors.end());
    pp.dataFileCalls.resize(callCount);
    if (!parse_rejected_data_file(pp)) {
      lowerer.reportParseError(error);
    }
    return;
  }
  for (Item* i : *items) {
