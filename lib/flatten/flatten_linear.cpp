...
class DeferLinearRelations {
public:
  DeferLinearRelations(EnvI& env, bool defer) : _env(env), _previous(env.deferLinearRelations) {
    _env.deferLinearRelations = defer;
  }
  ~DeferLinearRelations() { _env.deferLinearRelations = _previous; }
  DeferLinearRelations(const DeferLinearRelations&) = delete;
  DeferLinearRelations& operator=(const DeferLinearRelations&) = delete;
...
/// Every relation a comparison can be built from, and what it is a comparison
/// of: the operator, and how many arguments the relation takes before the
/// variable a reified one is reified into.
///
/// Interning the names once means recognising a relation is a lookup on a
/// pointer rather than picking a name apart.
struct RelationInfo {
  BinOpType bot;
  unsigned int arity;
  /// Whether the relation holds in one direction only.
  bool halfReified;
  /// Whether this is the compiler's own form, waiting to be built.
  bool deferred;
};

const ASTStringMap<RelationInfo>& linear_relations() {
  static const ASTStringMap<RelationInfo>* relations = [] {
    auto* m = new ASTStringMap<RelationInfo>();
    const BinOpType bots[] = {BOT_LQ, BOT_LE, BOT_EQ, BOT_NQ};
    const char* rels[] = {"le", "lt", "eq", "ne"};
    // A scalar relation compares two expressions; a linear one takes the
    // expression apart into a coefficient and a variable array.
    const std::pair<const char*, unsigned int> heads[] = {
        {"lin_exp", 2}, {"int", 2}, {"float", 2}, {"int_lin", 3}, {"float_lin", 3}};
    for (const auto& head : heads) {
      for (unsigned int i = 0; i < 4; i++) {
        const std::string base = std::string(head.first) + "_" + rels[i];
        for (const char* suffix : {"", "_reif", "_imp"}) {
          m->emplace(ASTString(base + suffix),
                     RelationInfo{bots[i], head.second, std::string(suffix) == "_imp",
                                  std::string(head.first) == "lin_exp"});
        }
      }
    }
    return m;
...
ASTString deferred_relation_id(BinOpType bot, bool& swapOperands) {
  static const ASTString le("lin_exp_le");
  static const ASTString lt("lin_exp_lt");
  static const ASTString eq("lin_exp_eq");
  static const ASTString ne("lin_exp_ne");
  swapOperands = bot == BOT_GR || bot == BOT_GQ;
  switch (bot) {
    case BOT_EQ:
      return eq;
    case BOT_NQ:
...
bool is_deferred_relation(const ASTString& id) {
  auto it = linear_relations().find(id);
  return it != linear_relations().end() && it->second.deferred;
}
...
/// Reconstruct the comparison represented by \a c and find its reification variable.
BinOp* linear_relation(EnvI& env, Call* c, VarDecl*& reifiedInto) {
  const auto& relations = linear_relations();
  auto it = relations.find(c->id());
  if (it == relations.end()) {
    return nullptr;
  }
  const unsigned int arity = it->second.arity;
  ...
  auto* bo = new BinOp(Expression::loc(c), lhs, it->second.bot, c->arg(arity - 1));
  bo->type(Type::varbool());
  return bo;
}

/// Rebuild \a c as \a e after naming decisions are final.
void rebuild_linear(EnvI& env, Item* i, Call* c, Expression* e, VarDecl* reifiedInto) {
  ...
  if (r != env.constants.varTrue &&
    ...
    auto rel = linear_relations().find(c->id());
    if (rel != linear_relations().end() && rel->second.halfReified) {
  ...
  // Keep this item counted as a reader until naming decisions are made.
  {
    ...
    // deferring it again.
    DeferLinearRelations defer(env, false);
...
void inline_linear_definition(EnvI& env, VarDeclI* vdi) {
  ...
  for (Item* reader : readers) {
    ...
    if (rc->id() == env.constants.ids.lin_exp) {
      rebuild_linear(env, reader, rc, rc, nullptr);
      continue;
    }
    VarDecl* r = nullptr;
    if (BinOp* bo = linear_relation(env, rc, r)) {
      rebuild_linear(env, reader, rc, bo, r);
    }
  }
}
...
/// Return the linear expression behind \a e, stopping at a named barrier.
/// A root definition may cross barriers because it supplies the name itself.
template <class Lit>
...
/// Like follow_id, but stop at named linear expressions.
